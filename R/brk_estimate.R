...
brk_estimate <- function(tickerlist, date, nob = 4, folder = Sys.getenv("GLOBAL"),
    ...
    for (i in 1:N) {
        ...
        tmp <- tmp %>% dplyr::filter(Type == 4 | Type == 5) %>%
                    dplyr::select(Secs, Midquote)
        tmp <- na.omit(tmp)
        tmp <- xts::as.xts(tmp$Midquote, order.by = sdate + tmp$Secs)
    ...
    groupval <- function(z) {
        ...
        for (w in 1:z) {
            ...
            rt_prices <- highfrequency::refreshTime(data_sorted[MYrange])
            xts::indexTZ(rt_prices) <- ""
            ...
            omegahat2 <- unlist(lapply(rt_prices, function(prices) mean(sapply(1:25,
                omega, prices))))
