...
readin_lobster <- function(ticker, date, nlevels = 1, output = "ALL", folder = ".") {
    ...
    m <- transform(m_raw, Price = Price/10000, Time = as.POSIXct(trunc(as.POSIXct(date), 
        units = "days") + Secs))
    if (!identical(output, "MB")) {
        ...
        o <- as.data.frame(o_raw) %>% dplyr::mutate_each(dplyr::funs(./10000), seq(from = 1, 
            to = nlevels * 4, by = 2)) %>% transform(Spread = Askp1 - Bidp1, Midquote = Bidp1 + 
            (Askp1 - Bidp1)/2)
    }
